...
public class ProtocolDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf in, List<Object> outList) throws Exception {
        try {
            ...
            log.info(message);
            FirmwareUpgradeWebSocket.SEND_MESSAGE_QUEUE.add(message);
    ...
    private void decodeMsgA5(byte[] buffer) {
        ...
        if (length == (byte) 0x1A) {
            ...
            FirmwareUpgradeWebSocket.SEND_MESSAGE_QUEUE.add("VERSION;;" + version);
        }
    }
    ...
    private boolean checkSum(byte[] buffer) {
        ...
        if (!b)
            FirmwareUpgradeWebSocket.SEND_MESSAGE_QUEUE.add(String.format("校验和应为：%s，实际是：%s", ByteUtils.toHexPrettyString(new byte[]{(byte) ((byte) 0xFF & summed)}), ByteUtils.toHexPrettyString(new byte[]{buffer[2]})));
        return b;
    }
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        FirmwareUpgradeWebSocket.SEND_MESSAGE_QUEUE.add(msg);
